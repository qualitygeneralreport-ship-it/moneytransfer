...
const waveComList = ref([]);
const accounts = ref([]);
const customers = ref([]);
const waveList = ref([]);

const fetchWaveCom = async () => {
  try {
    const data = await api.get('/commissions/');
    waveComList.value = data.filter(c => c.provider === 'WAVE');
  } catch (error) {
    console.error('Error fetching WaveCom:', error);
  }
};
...
const fetchAccounts = async () => {
  try {
    ...
    if (Array.isArray(data)) {
      ...
      if (wavePartnerAccounts.value.length > 0) form.value.cardSelect = wavePartnerAccounts.value[0];
...
const fetchWaveList = async () => {
  try {
    const data = await api.get('/transactions/wave/');
    if (Array.isArray(data)) {
      waveList.value = data;
    }
  } catch (error) {
...
const wavePartnerAccounts = computed(() => 
  accounts.value.filter(acc => acc[0] === 'WavePartner').map(acc => acc[1])
);

const otherAccounts = computed(() => 
  accounts.value.filter(acc => acc[0] !== 'WavePartner' && acc[0] !== 'TruePartner').map(acc => acc[1])
);

const reversedWaveList = computed(() => {
  return [...waveList.value].reverse();
});
...
const cashInCommission = computed(() => {
...
watch(cashInCommission, (val) => { form.value.cashInCom = val; });
...
const resetForm = () => {
  ...
  form.value = {
    cardSelect: wavePartnerAccounts.value[0] || '',
    accountName: otherAccounts.value[0] || '',
    customer: defaultCust?.[0] || '',
    phone: defaultCust?.[1] || '',
    transfer: '',
    transferPercent: '',
    cashOut: '',
...
const saveData = async () => {
  ...
  try {
    ...
    await api.post('/transactions/wave/', {
      ...
      cash_in_amount: form.value.cashIn || '0.00',
      cash_in_com: form.value.cashInCom || '0.00',
      balance_amount: form.value.balance || '0.00',
      service_fee: form.value.fee || '0.00',
      note: form.value.note
    });
    alert('Wave transaction saved successfully!');
    resetForm();
    fetchWaveList();
  } catch (error) {
...
const deleteTransaction = async (tx) => {
  ...
  isSubmitting.value = true;
  try {
    await api.delete(`/transactions/wave/${tx.id}/`);
    await fetchWaveList();
  ...
  <div class="container">
    <div class="form-card">
      <div class="header">
        ...
        <span class="header-title">WAVE MONEY TRANSACTION</span>
      ...
      <div class="content">
        ...
        <div class="top-row">
          <div class="input-row no-margin flex-1">
            ...
            <select v-model="form.cardSelect">
              <option v-for="name in wavePartnerAccounts" :key="name" :value="name">{{ name }}</option>
            </select>
          </div>
          <div class="input-row no-margin flex-1">
        ...
        <div class="transaction-selector">
          ...
          <label class="radio-label"><input type="radio" v-model="transactionType" value="cashIn"> ငွေသွင်း</label>
        ...
        <div v-if="transactionType === 'cashIn'" class="input-row">
          <label>ငွေသွင်း</label>
          <div class="split-inputs">
            <input type="number" class="white-bg" v-model="form.cashIn" placeholder="ပမာဏ">
            <input type="number" v-model="form.cashInCom" class="red-border-input" placeholder="Com">
          </div>
        </div>
    ...
    <div class="list-card">
      <div class="header">WAVE MONEY TRANSACTION LIST</div>
      <div class="table-container">
        <table>
          <thead>
            <tr>
              <th>Type</th><th>Customer</th><th>Phone</th><th>Transfer</th><th>Cash Out</th><th>Cash In</th><th>Fee</th><th>Time</th><th>Actions</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(tx, index) in reversedWaveList" :key="tx.id">
              <td>{{ tx.transaction_type }}</td>
              <td>{{ customers.find(c => c[2] === tx.customer)?.[0] || 'Unknown' }}</td>
              <td>{{ customers.find(c => c[2] === tx.customer)?.[1] || '' }}</td>
              <td>{{ tx.transfer_amount }}</td>
              <td>{{ tx.cash_out_amount }}</td>
              <td>{{ tx.cash_in_amount }}</td>
              <td>{{ tx.service_fee }}</td>
              <td>{{ tx.created_at ? new Date(tx.created_at).toLocaleTimeString() : '' }}</td>
              <td class="action-cell">
                <button class="delete-icon-btn" @click="deleteTransaction(tx)" :disabled="isSubmitting">{{ isSubmitting ? '⏳' : '↩️' }}</button>
              </td>
            </tr>
            <tr v-if="waveList.length === 0"><td colspan="9" style="text-align: center;">No transactions found.</td></tr>
          </tbody>
        </table>
      </div>
      ...
      <div class="mobile-card-list">
        <div v-for="(tx, index) in reversedWaveList" :key="tx.id" class="mobile-card">
          ...
          <div class="card-body">
            <p class="customer-info"><strong>👤 {{ customers.find(c => c[2] === tx.customer)?.[0] || 'Unknown' }}</strong> ({{ customers.find(c => c[2] === tx.customer)?.[1] || '' }})</p>
            <div class="card-grid">
              <div><span>Card:</span> {{ accounts.find(a => a[2] === tx.agent_card_account)?.[1] }}</div>
              <div><span>Acc:</span> {{ accounts.find(a => a[2] === tx.cash_account)?.[1] }}</div>
              <div v-if="tx.transfer_amount && parseFloat(tx.transfer_amount) != 0"><span>လွှဲငွေ:</span> {{ tx.transfer_amount }}</div>
              <div v-if="tx.cash_out_amount && parseFloat(tx.cash_out_amount) != 0"><span>ငွေထုတ်:</span> {{ tx.cash_out_amount }}</div>
              <div v-if="tx.cash_in_amount && parseFloat(tx.cash_in_amount) != 0"><span>ငွေသွင်း:</span> {{ tx.cash_in_amount }}</div>
              <div><span>ဝန်ဆောင်ခ:</span> {{ tx.service_fee }}</div>
            </div>
          </div>
          <button class="card-delete-btn" @click="deleteTransaction(tx)" :disabled="isSubmitting">
            {{ isSubmitting ? '⏳' : '↩️ Reverse' }}
          </button>
        </div>
      </div>
    </div>
